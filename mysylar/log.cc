...
void FileLogAppender::log(std::shared_ptr<Logger> ptr, LogLevel::Level level,const LogEvent::ptr event){
    if(level >= m_level){
        m_filestream << m_formatter.format(logger, level, event);
    }
}

void FilLogAppender::reopen(){
    if(m_filestream){
        m_filestream.close();
    }
...
void StdLogAppender::log(std::shared_ptr<Logger> ptr, LogLevel::Level level,const LogEvent::ptr event){
    if(level >= m_level){
...
void LogFormatter::init(){
    //str,foemat,type
    std::vocter<std::tuple<std::string,std::string,int>> vec;
    std::string nstr;
